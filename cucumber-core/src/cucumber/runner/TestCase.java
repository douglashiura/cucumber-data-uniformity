...
final class TestCase implements cucumber.api.TestCase {
	...
	public TestCase(List<PickleStepTestStep> testSteps, List<HookTestStep> beforeHooks, List<HookTestStep> afterHooks,
			PickleEvent pickleEvent, boolean dryRun) {
	...
	void run(EventBus bus) {
		...
		bus.send(new TestCaseFinished(stopTimeNanos, stopTimeMillis, this,
				new Result(scenario.getStatus(), stopTimeNanos - startTimeNanos, scenario.getError())));
	}
